...
class Motion_planning:
    ...
    def range_box_G(
        ...
        point2 = target
        obstacle1_box = np.array([0, 0, 0])  ##后面改成自动获取障碍物尺寸信息
        obstacle2_box = np.array([0, 0, 0])
            ...
            for j in [0, -1, 1]:
                for k in [0, -1, 1]:
                    ...
                    if (
                        ...
                        print(f"the ({x},{y},{z}) has out of range")
                        continue
                    ...
                    for x1 in range(min_x, max_x + 1):
                        for y1 in range(min_y, max_y + 1):
                            for z1 in range(min_z, max_z + 1):
                                if self.Map[x1][y1][z1] == 1:
                                    flag = 1
                                    print(
                                        f"the ({x},{y},{z}) has out of range due to an obstacle in range"
                                    )
                                    break
                            if flag == 1:
                                break
