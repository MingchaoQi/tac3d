...
def boundary_depth(cam_id):
    ...
    if GPU:
        gpuorcpu = "cuda"  # NVIDIA公司开发的一种编程模型和软件环境，它允许开发者使用C、C++、Python等高级语言来编写GPU程序。
    ...
    nn = gs3drecon.Reconstruction3D(dev)  # 重建3D图像
...
if __name__ == "__main__":
    argv = sys.argv[1:]  # [argument variables] 命令行调用时由系统传递给程序
    # sys.argv[0]是程序本身（全路径）。sys.argv[:]输出的是一个list，从程序外部获取参数
    device = "mini"
    try:
    ...
    cam_id = "GelSight Mini"  # 0 for sys
    dev = gsdevice.Camera(finger, cam_id)
    net_file_path = 'nnmini.pt'

    dev.connect()  # connect the dev, import base parameters

    ''' Load neural network '''
    model_file_path = path
    net_path = os.path.join(model_file_path, net_file_path)  # 路径拼接
    print('net path = ', net_path)
    ...
    try:
        while dev.while_condition:  # while_condition = 1
